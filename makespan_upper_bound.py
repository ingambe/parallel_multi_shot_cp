...
def solve_one_instance(instance_path: str, limit_time: int, hints: bool = True) -> Tuple[int, int, int, float]:
    ...
    return res.get_objective_value(), jobs_count, machines_count, time.time() - start, res.get_objective_bound()


def solver_multiple_instances(instances: List[str], limit_time: int, hints: bool = True, csv_name: str = 'results.csv'):
    import wandb
    wandb.init(project='CP_ICAPS', entity='cp_project', name=f'bound_single_solving_ibm_{limit_time // 60}min')
    random.seed(0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--time', type=int, default=60*60, help='Total time')
